...
class MockData {
  ...
  static List<Race> racesFor(
    int venue,
    String date, {
    int count = _defaultRaceCount,
  }) {
  ...
  static List<RaceEntry> entriesFor(int raceNo, [int venue = 1]) {
    ...
    return List.generate(
      ...
      (i) => RaceEntry(
        ...
        recentWinCount: (i + venue) % 3,
        winRate: 8.0 + i * 2.5,
        boatNo: 30 + (raceNo * 6 + i) % 40,
        motorNo: 10 + (raceNo * 3 + i) % 30,
        weight: 52.0 + (i * 1.5),
        avgStartTime: 0.18 + i * 0.01,
      ),
    );
  }

  static RaceResult raceResultFor(
    int raceNo, [
    List<RaceEntry>? realEntries,
    int venue = 1,
  ]) {
  ...
  static List<Map<String, dynamic>> raceRanksFor(
    int raceNo, [
    List<RaceEntry>? realEntries,
    int venue = 1,
  ]) {
    ...
    return List.generate(shuffled.length, (i) {
      ...
      return {
        ...
        'racer_grd': e.grade,
        'race_time':
            '0:${(38 + i * 2).toString().padLeft(2, '0')}.${(30 + i * 15 + venue * 5) % 100}',
